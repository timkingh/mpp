...
#define MODULE_TAG "hal_vp8d_vdpu1_reg"
#include <string.h>
...
#include "hal_vp8d_vdpu1_reg_table.h"
...
#endif
    if (NULL == ctx->regs) {
        ctx->regs = mpp_calloc_size(void, sizeof(VP8D_VDPU1_RegSet_t));
        if (NULL == ctx->regs) {
...
static MPP_RET hal_vp8_init_hwcfg(VP8DHalContext_t *ctx)
{
    ...
    VP8D_VDPU1_RegSet_t *reg = (VP8D_VDPU1_RegSet_t *)ctx->regs;

    FUN_T("FUN_IN");
    memset(reg, 0, sizeof(VP8D_VDPU1_RegSet_t));
...
{
    VP8D_VDPU1_RegSet_t *regs = (VP8D_VDPU1_RegSet_t *)ctx->regs;
...
MPP_RET hal_vp8d_vdpu1_start(void *hal, HalTaskInfo *task)
{
    ...
    for (i = 0; i < VP8D_VDPU1_REG_NUM; i++) {
        vp8h_dbg(VP8H_DBG_REG, "vp8d: regs[%02d]=%08X\n", i, *((RK_U32*)p));
        p += 4;
    }
    ret = VPUClientSendReg(ctx->vpu_socket, (RK_U32 *)regs, VP8D_VDPU1_REG_NUM);
    if (ret != 0) {
...
MPP_RET hal_vp8d_vdpu1_wait(void *hal, HalTaskInfo *task)
{
    ...
    FUN_T("FUN_IN");
    ret = VPUClientWaitResult(ctx->vpu_socket, (RK_U32 *)ctx->regs,
                              VP8D_VDPU1_REG_NUM, &cmd, &length);
    FUN_T("FUN_OUT");
#endif
    (void)hal;
    (void)task;
